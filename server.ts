...
// Autenticação Nativa Node.js + SQLite
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    // Inserir Perfil Vazio Associado
    db.prepare(`
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    const profile: any = db.prepare("SELECT * FROM profiles WHERE id = ?").get(user.id);
...
app.post("/api/auth/google", async (req, res) => {
  try {
    ...
    if (!email || !uid) {
      return res.status(400).json({ error: "Email e UID do Google obrigatórios." });
    }
    ...
    if (!user) {
      ...
      db.prepare("INSERT INTO users (id, email, password_hash) VALUES (?, ?, ?)").run(uid, email, password_hash);
    }

    // Check if profile exists in the profiles table
    let profile = db.prepare("SELECT * FROM profiles WHERE id = ?").get(uid);
    if (!profile) {
      // Create profile
      db.prepare(`
        INSERT INTO profiles (id, email, username, xp, streak, role, created_at) 
        VALUES (?, ?, ?, 150, 1, 'user', CURRENT_TIMESTAMP)
      `).run(uid, email, username || email.split("@")[0]);
      profile = db.prepare("SELECT * FROM profiles WHERE id = ?").get(uid);
    }

    const token = jwt.sign({ id: uid, email }, JWT_SECRET, { expiresIn: '7d' });
    res.json({ token, user: profile });
  } catch (err: any) {
    res.status(500).json({ error: "Erro ao processar login Google no servidor: " + err.message });
  }
});
...
app.get("/api/auth/me", async (req, res) => {
  try {
    ...
    const profile: any = db.prepare("SELECT * FROM profiles WHERE id = ?").get(decoded.id);
...
app.get("/api/profiles/:id", async (req, res) => {
  try {
    ...
    // Parse JSON fields
    if (profile.meals) profile.meals = JSON.parse(profile.meals);
    if (profile.custom_meals) profile.custom_meals = JSON.parse(profile.custom_meals);
...
app.put("/api/profiles/:id", async (req, res) => {
  try {
    ...
    for (const [key, value] of Object.entries(updates)) {
      ...
      if (key === 'meals' || key === 'custom_meals' || typeof value === 'object') {
    ...
    if (setClause.length === 0) return res.json({ message: "Nenhum campo para atualizar." });
    
    values.push(id);
    db.prepare(`UPDATE profiles SET ${setClause.join(", ")} WHERE id = ?`).run(...values);
...
app.post("/api/profiles", async (req, res) => {
  try {
    ...
    const placeStr = placeholders.join(', ');
    db.prepare(`INSERT INTO profiles (${colStr}) VALUES (${placeStr})`).run(...values);

    res.json({ success: true, message: "Perfil criado com sucesso" });
  } catch(err: any) {
    res.status(500).json({ error: "Erro ao criar perfil: " + err.message });
  }
});

// Food Logs Routes
app.get("/api/food_logs/:userId", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Erro ao buscar food logs: " + err.message });
  }
});

app.post("/api/food_logs", async (req, res) => {
  try {
    const { 
      user_id, food_id, meal_type, date, portion_amount, 
      name, calories, protein, carbs, fat, measure_unit, grams_per_unit, category 
    } = req.body;
    ...
    db.prepare(`
      INSERT INTO food_logs (
        id, user_id, food_id, meal_type, date, portion_amount, 
        name, calories, protein, carbs, fat, measure_unit, grams_per_unit, category, created_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
    `).run(
      id, user_id, food_id || null, meal_type, date, portion_amount || 1,
      name || null, calories || 0, protein || 0, carbs || 0, fat || 0, 
      measure_unit || null, grams_per_unit || 1, category || null
    );
    
    res.json({ id, success: true });
  } catch (err: any) {
    res.status(500).json({ error: "Erro ao criar food log: " + err.message });
  }
});

app.put("/api/food_logs/:id", async (req, res) => {
  try {
    const id = req.params.id;
    const updates = req.body;
    ...
    values.push(id);
    db.prepare(`UPDATE food_logs SET ${setClause.join(", ")} WHERE id = ?`).run(...values);
    
    res.json({ success: true, message: "Food log atualizado." });
  } catch (err: any) {
    res.status(500).json({ error: "Erro ao atualizar food log: " + err.message });
  }
});

app.delete("/api/food_logs/:id", async (req, res) => {
  try {
    const id = req.params.id;
    db.prepare("DELETE FROM food_logs WHERE id = ?").run(id);
    res.json({ success: true, message: "Food log removido." });
  } catch (err: any) {
    res.status(500).json({ error: "Erro ao remover food log: " + err.message });
  }
});

app.delete("/api/food_logs/user/:userId", async (req, res) => {
  try {
    db.prepare("DELETE FROM food_logs WHERE user_id = ?").run(req.params.userId);
    res.json({ success: true, message: "Food logs removidos." });
  } catch (err: any) {
    res.status(500).json({ error: "Erro ao remover food logs: " + err.message });
  }
});

// Water Logs Routes
app.get("/api/water_logs/:userId", async (req, res) => {
  try {
    const userId = req.params.userId;
    const { date } = req.query;
    ...
    const params: any[] = [userId];
    if (date) {
       // date format filtering based on 'YYYY-MM-DD' prefix match since logged_at might be an ISO string
       query += " AND logged_at LIKE ?";
       params.push(`${date}%`);
    }
    const logs = db.prepare(query).all(...params);
    res.json(logs);
  } catch (err: any) {
    res.status(500).json({ error: "Erro ao buscar water logs: " + err.message });
  }
});

app.post("/api/water_logs", async (req, res) => {
  try {
    const { user_id, amount, logged_at } = req.body;
    const id = crypto.randomUUID();
    db.prepare(`
      INSERT INTO water_logs (id, user_id, amount, logged_at) 
      VALUES (?, ?, ?, ?)
    `).run(id, user_id, amount, logged_at);
    res.json({ id, success: true });
  } catch (err: any) {
    res.status(500).json({ error: "Erro ao criar water log: " + err.message });
  }
});

app.delete("/api/water_logs/:id", async (req, res) => {
  try {
    const id = req.params.id;
    db.prepare("DELETE FROM water_logs WHERE id = ?").run(id);
    res.json({ success: true, message: "Water log removido." });
  } catch (err: any) {
    res.status(500).json({ error: "Erro ao remover water log: " + err.message });
  }
});

app.delete("/api/water_logs/user/:userId", async (req, res) => {
  try {
    db.prepare("DELETE FROM water_logs WHERE user_id = ?").run(req.params.userId);
    res.json({ success: true, message: "Water logs removidos." });
  } catch (err: any) {
    res.status(500).json({ error: "Erro ao remover water logs: " + err.message });
  }
});

// Workouts Routes
app.get("/api/workout_profiles/:userId", async (req, res) => {
  try {
    const userId = req.params.userId;
    const profile = db.prepare("SELECT * FROM workout_profiles WHERE user_id = ?").get(userId);
    if (!profile) return res.status(404).json({ error: "Perfil de treino não encontrado." });
    
    if (profile.muscle_fatigue) profile.muscle_fatigue = JSON.parse(profile.muscle_fatigue);
    res.json(profile);
  } catch (err: any) {
    res.status(500).json({ error: "Erro ao buscar perfil de treino: " + err.message });
  }
});

app.put("/api/workout_profiles/:userId", async (req, res) => {
  try {
    const userId = req.params.userId;
    const updates = req.body;
    ...
    for (const [key, value] of Object.entries(updates)) {
       ...
       const val = (key === 'muscle_fatigue' || typeof value === 'object') ? JSON.stringify(value) : value;
       
       fieldsToInsert.push(key);
       valuesToInsert.push(val);
       placeholdersInsert.push('?');
       
       setClause.push(`${key} = ?`);
       valuesUpdate.push(val);
    }
    
    if (!existing) {
       fieldsToInsert.push('created_at');
       valuesToInsert.push(new Date().toISOString());
       placeholdersInsert.push('?');
       db.prepare(`INSERT INTO workout_profiles (${fieldsToInsert.join(', ')}) VALUES (${placeholdersInsert.join(', ')})`).run(...valuesToInsert);
    } else {
       if (setClause.length > 0) {
          valuesUpdate.push(userId);
          db.prepare(`UPDATE workout_profiles SET ${setClause.join(', ')} WHERE user_id = ?`).run(...valuesUpdate);
       }
    }
    res.json({ success: true, message: "Perfil de treino salvo." });
  } catch (err: any) {
    res.status(500).json({ error: "Erro ao salvar perfil de treino: " + err.message });
  }
});

app.get("/api/workout_routines/:userId", async (req, res) => {
  try {
    const userId = req.params.userId;
    const routine = db.prepare("SELECT * FROM workout_routines WHERE user_id = ?").get(userId);
    if (!routine) return res.status(404).json({ error: "Rotina não encontrada." });
    
    if (routine.workouts) routine.workouts = JSON.parse(routine.workouts);
    res.json(routine);
  } catch (err: any) {
    res.status(500).json({ error: "Erro ao buscar rotina: " + err.message });
  }
});

app.put("/api/workout_routines/:userId", async (req, res) => {
  try {
    const userId = req.params.userId;
    ...
    const existing = db.prepare("SELECT * FROM workout_routines WHERE user_id = ?").get(userId);
    if (existing) {
      db.prepare(`UPDATE workout_routines SET name = ?, description = ?, workouts = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?`).run(name, description, workoutsJson, userId);
    } else {
      db.prepare(`INSERT INTO workout_routines (user_id, name, description, workouts, created_at, updated_at) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`).run(userId, name, description, workoutsJson);
    }
    
    res.json({ success: true, message: "Rotina salva." });
  } catch (err: any) {
    res.status(500).json({ error: "Erro ao salvar rotina: " + err.message });
  }
});

app.get("/api/exercise_logs/:userId", async (req, res) => {
  try {
    const logs = db.prepare("SELECT * FROM exercise_logs WHERE user_id = ? ORDER BY date DESC, created_at DESC").all(req.params.userId);
    logs.forEach(l => l.sets = JSON.parse(l.sets));
    res.json(logs);
  } catch (err: any) {
    res.status(500).json({ error: "Erro ao buscar logs de exercícios: " + err.message });
  }
});

app.post("/api/exercise_logs", async (req, res) => {
  try {
    const { user_id, routine_id, workout_id, exercise_id, date, sets } = req.body;
    const id = crypto.randomUUID();
    db.prepare(`
      INSERT INTO exercise_logs (id, user_id, routine_id, workout_id, exercise_id, date, sets)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(id, user_id, routine_id, workout_id, exercise_id, date, JSON.stringify(sets || []));
    res.json({ id, success: true });
  } catch (err: any) {
    res.status(500).json({ error: "Erro ao criar log de exercício: " + err.message });
  }
});

app.delete("/api/exercise_logs/user/:userId", async (req, res) => {
  try {
    db.prepare("DELETE FROM exercise_logs WHERE user_id = ?").run(req.params.userId);
...
app.delete("/api/exercise_logs/:id", async (req, res) => {
  try {
    db.prepare("DELETE FROM exercise_logs WHERE id = ?").run(req.params.id);
    res.json({ success: true });
  } catch (err: any) {
    res.status(500).json({ error: "Erro ao deletar log: " + err.message });
  }
});
...
app.get("/api/configs/:id", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Erro ao buscar config: " + err.message });
  }
});
...
app.put("/api/configs/:id", async (req, res) => {
  try {
    ...
    const existing = db.prepare("SELECT * FROM configs WHERE id = ?").get(id);
    
    if (existing) {
       db.prepare("UPDATE configs SET value = ? WHERE id = ?").run(value, id);
    } else {
       db.prepare("INSERT INTO configs (id, value) VALUES (?, ?)").run(id, value);
    }
    
    res.json({ success: true });
  } catch (err: any) {
    res.status(500).json({ error: "Erro ao salvar config: " + err.message });
  }
});

app.get("/api/profiles", async (req, res) => {
  try {
    const { league, username } = req.query;
    ...
    queryStr += " ORDER BY xp DESC LIMIT 100";
    const profiles = db.prepare(queryStr).all(...params);
    res.json(profiles);
  } catch (err: any) {
    res.status(500).json({ error: "Erro ao buscar perfis: " + err.message });
  }
});
...
app.delete("/api/profiles/:id", async (req, res) => {
  try {
    ...
    res.json({ success: true, message: "Perfil apagado." });
  } catch (err: any) {
    res.status(500).json({ error: "Erro: " + err.message });
  }
});

// Admin config and stats
app.get("/api/admin/stats", async (req, res) => {
  try {
    const usersCount = db.prepare("SELECT COUNT(*) as count FROM users").get() as any;
    const foodLogsCount = db.prepare("SELECT COUNT(*) as count FROM food_logs").get() as any;
    res.json({ usersCount: usersCount.count, foodLogsCount: foodLogsCount.count });
  } catch (err: any) {
...
app.get("/api/admin/whatsapp_logs", async (req, res) => {
  try {
    ...
    const isAdmin = await checkIsAdmin(adminUserId, adminEmail);
    if (!isAdmin) {
      return res.status(200).json({ success: false, error: "Acesso negado." });
    }

    const snapshot = await firestore.collection("whatsapp_webhook_logs")
